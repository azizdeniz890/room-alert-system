...
#BLUETOOTH BAĞLANTISI (ESP32)
try:
    bluetooth = serial.Serial("COM4", 9600)  # com portunu kendi sistemine göre ayarlayınız.
    time.sleep(2)
    print("[INFO] ESP32 Bluetooth bağlantısı kuruldu.")
except Exception as e:
    bluetooth = None
    print("[HATA] ESP32 bağlantısı kurulamadı:", e)

#MODEL YÜKLEME CUDA KONTROL
...
while True:
    ...
    if person_detected_now:
        ...
        if not person_currently_present:
            ...
            # ESP32'ye sinyal gönderme
            if bluetooth:
                try:
                    bluetooth.write(b'1')  # ESP32 '1' sinyali alınca ses verir / ekrana yazar
                    print("[INFO] ESP32'ye sinyal gönderildi.")
                except Exception as e:
                    print("[HATA] ESP32 sinyal gönderilemedi:", e)
...
if bluetooth:
    bluetooth.close()
    print("[INFO] ESP32 Bluetooth bağlantısı kapatıldı.")
